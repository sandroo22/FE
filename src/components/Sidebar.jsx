  ...
  return (
    <>
      <aside className="w-full md:w-64 lg:w-72 border-b md:border-b-0 md:border-r border-border bg-background p-4 flex flex-col gap-4 shrink-0 overflow-y-auto transform-gpu">
            ...
            <li key={lista.id}>
              {listaInModifica === lista.id ? (
                <div className="flex gap-1 items-center p-1">
                  <Input
                    value={testoModificaLista}
                    onChange={(e) => setTestoModificaLista(e.target.value)}
                  ...
                  <Button
                    ...
                    onClick={() => salvaModificaLista(lista.id)}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => setListaInModifica(null)}
                  >
                ...
                >
                  ...
                  {!lista.is_default && selectedListaId === lista.id && (
                    <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <FaEdit
                        className="h-3 w-3 text-muted-foreground hover:text-blue-500"
                        onClick={(e) => {
                          e.stopPropagation();
                          setListaInModifica(lista.id);
                          setTestoModificaLista(lista.nome);
                        }}
                      />
                      <FaTrash
                        className="h-3 w-3 text-muted-foreground hover:text-destructive"
                        onClick={(e) => apriAlertEliminazione(e, lista.id)}
                      />
                    </div>
                  )}
                </div>
        ...
        <form
          onSubmit={creaNuovaLista}
          className="mt-4 pt-4 border-t flex gap-2"
        >
          <Input
            placeholder="Nuova cartella..."
            value={nuovaListaNome}
            onChange={(e) => setNuovaListaNome(e.target.value)}
      ...
      {/* ALERT DI CONFERMA SHADCN */}
      <AlertDialog open={isAlertOpen} onOpenChange={setIsAlertOpen}>
        <AlertDialogContent className="w-[95vw] sm:w-full rounded-lg">
          <AlertDialogHeader>
            <AlertDialogTitle>Sei assolutamente sicuro?</AlertDialogTitle>
            <AlertDialogDescription>
              Questa azione eliminerà la cartella. I film al suo interno non
              andranno persi, ma verranno spostati in automatico nella cartella{" "}
